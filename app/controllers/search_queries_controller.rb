class SearchQueriesController < ApplicationController
  ...
  def create
    ...
    latest_complete_query = SearchQuery.where(user_identifier: user_ip).where.not(query: nil).order(created_at: :desc).first
  ...
  def get_similar_queries
    ...
    similar_queries = SearchQuery
                    .where(user_identifier: user_ip)
                    .where("REPLACE(LOWER(query), ' ', '') LIKE ?", "%#{query}%")
                    .pluck(:query)
    
    puts "Generated SQL Query: #{SearchQuery.where(user_identifier: user_ip).where("REPLACE(LOWER(query), ' ', '') LIKE ?", "%#{query}%").to_sql}"
  ...
  def user_identifier
    cookies.permanent.signed[:user_identifier] ||= SecureRandom.uuid
  end
  ...
  def create_new_search_query(query, user_ip)
    ...
    previous_queries = SearchQuery.where(user_identifier: user_ip)
    ...
    unless SearchQuery.exists?(query: query)
      @search_query = SearchQuery.new(query: query, user_identifier: user_ip)
      @search_query.save
    end
  end
  ...
  def clean_up_incomplete_queries(user_ip)
    ...
    queries = SearchQuery.where(user_identifier: user_ip).to_a
